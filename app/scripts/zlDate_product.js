...
var dateUtil = {
    ...
    getSelect: function () {
      var dt = beginDate;
      obj.year = parseInt(dt.split("-")[0]);
      obj.month = parseInt(dt.split("-")[1]);
      obj.day = parseInt(dt.split("-")[2]);
    },
    ...
    getPrice: function (day) {
        ...
        for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].date == dt) {
                return obj.priceArr[i].date;
            }
        }
        if(typeof($("#isModel").val())!="undefined"){
          return 0;
        }
        return -1;
    },
...
$(document).bind("click", function (event) {
    ...
    while (elem) {
        if (elem.id == "calendar_choose"||elem.id=="beginDate") {
            return;
        }
